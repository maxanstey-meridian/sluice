...
public sealed class SluiceGenerator : IIncrementalGenerator
{
    ...
    private sealed record DiagnosticInfo(string Id, string Message);
    ...
    private static InterfaceData? Transform(GeneratorAttributeSyntaxContext ctx)
    {
        ...
        foreach (var member in typeSymbol.GetMembers())
        {
            ...
            {
                bool implementsIResourceKey =
                    keyParam.Type.Equals(resourceKeySymbol, SymbolEqualityComparer.Default)
            ...
            else if (isWrite)
            {
                ...
                if (hasResultKey && valueType is not null && resultTypeSymbol is not null)
                {
                    ...
                    foreach (var (_, _, rk) in tempResources)
                    {
                        ...
                        {
                            implementsIResourceKey =
                                propNamed.Equals(resourceKeySymbol, SymbolEqualityComparer.Default)
